...
/// template class for unary RPC client reactor
...
class ProxyUnaryReactor : public grpc::ClientUnaryReactor {
 public:
  /// Callback signature for ClientReadReactor::OnDone event
  ...
  /// function at any other moment is meaningles.
...
/// template class for stream-reader RPC client reactor
/// @tparam ResponseT type of protobuf message the RPC handles
template <class ResponseT>
requires std::derived_from<ResponseT, google::protobuf::Message>
class ProxyReadReactor : public grpc::ClientReadReactor<ResponseT> {
 public:
  /// Callback signature for ClientReadReactor::OnReadDone event with positive OK flag
  /// TODO(ajcote) Would be nicer to have these callback signatures outside of the template class. But to achieve
  ///  this, it means the callback must also be generic about the protobuf response message.
  using OnReadDoneOkCallback = std::function<bool(ProxyReadReactor*, const ResponseT&)>;
  /// Callback signature for ClientReadReactor::OnReadDone event with negative OK flag
  ...
  /// RPC is meant to be overwritten at each incoming resoinse, so the swap is a
 ...
 protected:
  ...
  /// This OnReadDone is also the relevent event acknowledging the response reception.
  /// Based on the valus of the `ok` flag, the OnReadDoneOkCallback or OnReadDoneNOkCallback
  /// is called.
  ...
  /// This event function is called by gRPC when the RPC is done. The OnDoneCallback
  /// is then called, but on the same gRPC thread. The received status
  /// is also copied into the reactor.
  /// @param status info coming from gRPC
  void OnDone(const grpc::Status& status) override {
