...
// would free it twice.
constexpr auto kOnReadDoneOk{"RouteChatContOnReadDoneOk"};
constexpr auto kOnReadDoneNOk{"RouteChatContOnReadDoneNOk"};
constexpr auto kOnWriteDone{"RouteChatContOnWriteDone"};
constexpr auto kOnDone{"RouteChatContOnDone"};
}  // anonymous namespace
...
void RouteChat::Start(std::vector<routeguide::RouteNote> notes) {
  ...
  Callbacks cbs;
  // TriggerEvent: OnReadDoneOk, handing the message ownership to the queue
  ...
  reactor_ = std::make_unique<ClientReactor>(stub_, CreateClientContext(), std::move(cbs), ReadPacing::kContinuous);
